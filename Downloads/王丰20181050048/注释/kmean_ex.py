...
def kMeans(dataSet, k, distMeas=distEclud, createCent=randCent):
    m = shape(dataSet)[0]#读取行数
    clusterAssment = mat(zeros((m,2)))
    centroids = createCent(dataSet, k)#随机产生质心初始化
    clusterChanged = True   #布尔赋值决定是否确定最小
    while clusterChanged:
        ...
        for i in range(m):
            ...
            for j in range(k):
                ...
                if distJI < minDist:
                    minDist = distJI; minIndex = j  #找到更小的质心
            if clusterAssment[i,0] != minIndex: #如果当前质心不是最小距离的质心则更新质心
                clusterChanged = True #循环过程直到当前质心为最小值
            clusterAssment[i,:] = minIndex,minDist**2   #当前的质心，当前最小距离的平方
        print(centroids)#显示簇
        for cent in range(k):
...
mycentroids, clustAssing = kMeans(dataMat, 4) #该算法四个质心
